...
export default function CountdownEditPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [config, setConfig] = useState<CountdownConfigState>(DEFAULT_COUNTDOWN_CONFIG);
  ...
  useEffect(() => {
    if (component) {
      ...
      setConfig({
        backgroundImage: (rawConfig.backgroundImage as string | undefined) ?? '',
        ...
        discountText: (rawConfig.discountText as string | undefined) ?? '',
        endDate: (rawConfig.endDate as string | undefined) ?? '',
        heading: (rawConfig.heading as string | undefined) ?? '',
        showDays: rawConfig.showDays !== false,
        showHours: rawConfig.showHours !== false,
        showMinutes: rawConfig.showMinutes !== false,
        showSeconds: rawConfig.showSeconds !== false,
        style: (rawConfig.style as CountdownStyle) || 'banner',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success('Đã cập nhật Countdown');
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-6 pb-20">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <ConfigJsonForm value={config} onChange={(next) =>{  setConfig(next as CountdownConfigState); }} title="Cấu hình Countdown" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr,420px] gap-6">
          ...
          <div className="lg:sticky lg:top-6 lg:self-start">
            <CountdownPreview
              ...
              brandColor={primary}
              secondary={secondary}
              selectedStyle={config.style as CountdownStyle}
